...
- `sourceObservedAt` is currently generated by the API after the database projection and audit write. It is a **response-generation timestamp**, not a database snapshot timestamp, event occurrence time or proof of current clinical state. Preserve this distinction in any CF-J01B UI; a future contract revision may rename the field.
...
Integration tests cover own and non-assigned clinician, wrong clinical/admin roles, foreign tenancy, absent session, draft invisibility, no-arrival, published note, initial receipt, late corrections, partial receipt, repeated reads without side effects, cancelled appointment, inactive clinician profile and suspended organization. A dedicated unit test covers contradictory snapshots and source outage → 503. A separate PostgreSQL/MockMvc regression verifies that appointment reassignment immediately removes the former practitioner's access, permits the newly assigned practitioner while authorized, and denies access again when that practitioner's tenant membership is revoked.
